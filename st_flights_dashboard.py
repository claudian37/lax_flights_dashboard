...
st.set_page_config(
	layout="wide", 
	page_title="LAX Flight Departures", 
	page_icon=":airplane:"
)
...
def group_data_by_terminal(
	...
	grouped = data.groupby(
		["dep_terminal"]
	).agg(count_flights=("flight_iata", "nunique")).reset_index()
	return grouped

# CALCULATE GROUPED DATA BY AIRLINE
@st.experimental_memo
def group_data_by_airline(
	data: pd.DataFrame,
	departure_hour: Optional[int]
):
	data = filter_data(data, departure_hour)
	grouped = data.groupby(
		["dep_terminal", "airline_iata"]
	).agg(count_flights=("flight_iata", "nunique")).reset_index()
	grouped["airport"] = "LAX"
	return grouped
...
st.markdown("***Adjust slider to see how LAX flight departures differ by terminal at different hours of the day.***")

hour_selected = st.slider(
	"Select hour of flight departure", 0, 23, key="departure_hour", on_change=update_query_params
)
...
chart_dep = alt.Chart(chart_data).mark_area(
...
chart_terminal = alt.Chart(grouped_by_terminal_data).mark_bar().encode(
...
# PLOT CHARTS
with row2_1:
	st.write(f"**Breakdown of departures by terminal & airline between {hour_selected}:00 and {(hour_selected + 1) % 24}:00**")
	if grouped_by_airline_data.empty:
		st.write(f"***No Flights leaving LAX between {hour_selected}:00 and {(hour_selected + 1) % 24}:00***")
	else:
		fig = px.sunburst(
